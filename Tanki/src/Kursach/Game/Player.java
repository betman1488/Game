...
public class Player extends Entity {
    ...
    private enum Heading{ //направление
        NORTH(0 * SPRITE_SCALE, 0 * SPRITE_SCALE, 1 *SPRITE_SCALE, 1 * SPRITE_SCALE), //север
    ...
    public void update(Input input) {
        ...
        if (input.getKay(KeyEvent.VK_UP)) {
            newY -= speed;
            heading = Heading.NORTH;
        } else if (input.getKay(KeyEvent.VK_RIGHT)) {
            newX += speed;
            heading = Heading.EAST;
        } else if (input.getKay(KeyEvent.VK_DOWN)) {
            newY += speed;
            heading = Heading.SOUTH;
        } else if (input.getKay(KeyEvent.VK_LEFT)) {
            newX -= speed;
